...
reglas_empleo = {
    "bajo_riesgo": rule_engine.Rule("empleo == 'fijo' and anios_experiencia > 3"),
    "riesgo_moderado": rule_engine.Rule("empleo == 'fijo' and anios_experiencia <= 2"),
...
def calcular_puntuacion(cliente):
    ...
    for nivel, regla in reglas_garantias.items():
        if regla.matches(cliente):
            puntuacion += {"bajo_riesgo": -50, "riesgo_moderado": 5, "alto_riesgo": 10}[nivel]

    for nivel, regla in reglas_prestamos_previos.items():
        if regla.matches(cliente):
            puntuacion += {"bajo_riesgo": 0, "riesgo_moderado": 10, "alto_riesgo": 20}[nivel]

    # Evaluar regla de "buena persona"
    puntos_buena_persona = 0

    if reglas_prestamos_previos["bajo_riesgo"].matches(cliente):
        puntos_buena_persona += 1

    if reglas_empleo["bajo_riesgo"].matches(cliente):
        puntos_buena_persona += 1

    if reglas_dti["bajo_riesgo"].matches(cliente):
        puntos_buena_persona += 1

    if (reglas_garantias["bajo_riesgo"].matches(cliente) or
        reglas_garantias["riesgo_moderado"].matches(cliente)):
        puntos_buena_persona += 1

    if puntos_buena_persona >= 3:
        puntuacion -= 10
    else:
        puntuacion += 5
...
@app.route("/")
def home():
    return render_template("index_S2.html")
